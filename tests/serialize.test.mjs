...
test("compound paths own real children, cut even-odd holes, and release", () => {
  ...
  const outerEffects = [{ id: "fx_outer", enabled: true, type: "blur", radius: 1 }];
  doc.nodes.outer = {
  ...
  const childEffects = [{ id: "fx_child", enabled: true, type: "blur", radius: 2 }];
  const compoundEffects = [{ id: "fx_compound", enabled: true, type: "blur", radius: 9 }];
