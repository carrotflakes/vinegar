...
export function createShapeActions({ set, get, transact, replaceDocumentWithoutHistory }: StoreCtx): ShapeActions {
  ...
  return {
    ...
    setSelectedFillRule: (rule) => { const doc = get().doc; const nodes = { ...doc.nodes }; let changed = false; for (const id of selectionRoots(doc, get().selection)) { const shape = nodes[id]; if (!isShape(shape) || shape.type !== "path" || shape.fillRule === rule) continue; nodes[id] = { ...shape, fillRule: rule }; changed = true; } if (changed) transact({ ...doc, nodes }, { label: "Edit fill rule" }); },
