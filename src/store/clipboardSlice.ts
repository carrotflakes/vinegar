...
export function createClipboardActions({ set, get, transact }: StoreCtx): ClipboardActions {
  return {
    ...
    pastePayload: (clipboard, at) => {
      ...
      // Scripts, assets and variables the destination lacks come from the
      ...
      const { nodes: reattached, scripts, assets, vars, varOrder } = resolved;
      ...
      const next = appendToScope(
        { ...doc, nodes: { ...doc.nodes, ...pasted.nodes }, scripts, assets, vars, varOrder },
        scope,
        pasted.rootIds
      );
