...
export function createHistory(set: StoreSet, get: StoreGet): HistorySlice {
  ...
  const actions: HistoryActions = {
    ...
    cancelInteraction: () => { resetCoalesce(); const { _pending, _dirty } = get(); if (_pending && _dirty) set({ doc: _pending, ...clearTransient, selectedArtboardId: get().selectedArtboardId }); set({ _pending: null, _dirty: false }); },
    undo: () => { resetCoalesce(); const { history, doc } = get(); if (!history.past.length) return; const past = [...history.past], prev = past.pop()!; set({ doc: prev, history: { past, future: [clone(doc), ...history.future].slice(0, historyLimit()) }, selection: get().selection.filter((id) => !!prev.nodes[id]), editingSymbols: get().editingSymbols.filter((id) => !!prev.symbols[id]), ...clearTransient, selectedArtboardId: prev.artboards.some((ab) => ab.id === get().selectedArtboardId) ? get().selectedArtboardId : null }); },
    redo: () => { resetCoalesce(); const { history, doc } = get(); if (!history.future.length) return; const [next, ...future] = history.future; set({ doc: next, history: { past: [...history.past, clone(doc)].slice(-historyLimit()), future }, selection: get().selection.filter((id) => !!next.nodes[id]), editingSymbols: get().editingSymbols.filter((id) => !!next.symbols[id]), ...clearTransient, selectedArtboardId: next.artboards.some((ab) => ab.id === get().selectedArtboardId) ? get().selectedArtboardId : null }); },
  };
