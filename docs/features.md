...
- **Layers panel selection**: Shift+click takes a contiguous range and Ctrl/Cmd+click toggles one row (locked and hidden rows stay out of a range); ↑/↓ walk the rows of the focused list and ←/→ fold a container, dragging a selected row moves the whole selection in one undo step, and a selection made on the canvas scrolls its row into view, unfolding whatever hid it. `Enter` selects the contents of the selected containers, `Shift+Enter` their parent
