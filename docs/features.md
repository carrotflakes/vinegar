...
- Pencil: freehand strokes are simplified and smoothed into an editable Bézier path (tweak it with the Node tool); end near the start to close it (at the same distance the pen closes a draft, enlarged for touch) — the start point rings and the loop takes the current fill while the release would close it, and a closed stroke keeps that fill where an open one is a line with none. The point a stroke begins at snaps to guides and the grid (not to other shapes — a freehand start should only meet references you placed on purpose); nothing after it snaps. Live smoothing strength and the commit-time simplify tolerance are adjustable in the tool options (both shared with the brush). Start a stroke near an endpoint of a **selected** open path to continue (extend) it instead of drawing a new one
...
- **Document variables** (global colors + parameters, one panel): named values stored on the document. A *colour* variable holds any concrete paint (solid, gradient or pattern) that a fill/stroke can *reference* by id — edit it once and every use re-tints live; a *number* variable drives every field bound to it (stroke width, path-modifier params, built-in generator args). The **Variables panel** creates a colour from the selection, renames, applies it to the selection's fill/stroke, scrubs numbers, and deletes (detaching every use first — paint references bake, bound fields keep their number — so nothing dangles and nothing moves). The color popover links a paint to a variable or unlinks it; SVG export bakes references to concrete colors. See [global-colors.md](global-colors.md) and [parameters.md](parameters.md)
- **Non-destructive boolean** ("Combine (live)"): unite / subtract / intersect / exclude as a **modifier** rather than a bake. The other shape stays in the document — hidden, but selectable, movable and editable — and the combined outline re-resolves as it moves, for rendering, hit-testing, bounds and export alike. The modifier row picks its operand by clicking it on the canvas, reports a missing one instead of silently doing nothing, and *Apply modifiers* bakes the result when you want it flattened; the one-shot Pathfinder commands are unchanged. See [path-modifiers.md](path-modifiers.md)
- **Parametric symbols**: a symbol definition can declare **parameters** — promote any paint field to one from inside symbol-edit focus — and each placed instance overrides them on its own, so one definition paints differently per use while instances that override nothing follow the definition's default. Numeric symbol parameters are declarable but not overridable yet. See [parameters.md](parameters.md)
